  ...
  const handleLogin = useCallback(
    async (values, { setSubmitting }) => {
      ...
      if (lockoutTime && new Date() < new Date(lockoutTime)) {
        const remainingMins = Math.ceil((new Date(lockoutTime) - new Date()) / 60000);
        toast.error(`Too many failed attempts. Please try again in ${remainingMins} minute(s).`);
        setSubmitting(false);
        return;
      }
      ...
      <div className="login-wrapper">
        ...
        <div className="login-brand">
          <div className="brand-content">
            <h1 className="brand-title">TITech Community Capital</h1>
          ...
          <div className="login-card">
            ...
            {lockoutTime && new Date() < new Date(lockoutTime) && (
              <div className="lockout-warning" role="alert" aria-live="polite">
                <AlertCircle size={18} />
                <span>Account temporarily locked. Please try again in 15 minutes.</span>
              </div>
            )}

            {/* Show attempt count warning */}
            {!lockoutTime && attemptCount > 2 && attemptCount < MAX_ATTEMPTS && (
              <div className="attempt-warning" role="alert" aria-live="polite">
                <Info size={18} />
                <span>
                ...
                <Form className="login-form" noValidate>
                  ...
                  <div className="form-options">
                    <label className="remember-checkbox">
                      <Field type="checkbox" name="remember" disabled={loading} />
                      <span>Remember me</span>
                    </label>
                    <Link to="/forgot-password" className="forgot-link" tabIndex="0">
                      Forgot password?
                    </Link>
                  </div>
